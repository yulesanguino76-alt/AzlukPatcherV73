...
class ToolsViewModel : ViewModel() {
    ...
    fun scanWifi(ctx: Context) = viewModelScope.launch(Dispatchers.IO) {
        ...
        try {
            ...
            _state.update {
                it.copy(wifiEnabled = enabled)
            }

            if (!enabled) {
                _state.update {
                    it.copy(wifiScanning = false)
                }
                return@launch
            }
            ...
            val nets = results
                .map { r ->
                    ...
                    val ch = when {
                        r.frequency < 2484 ->
                            ((r.frequency - 2412) / 5) + 1

                        r.frequency == 2484 ->
                            14

                        else ->
                            ((r.frequency - 5180) / 5) + 36
                    }
    ...
    private fun fmt(b: Long) = when {
        ...
        b < 1024 * 1024 ->
            "%.1f KB".format(b / 1024f)

        b < 1024L * 1024 * 1024 ->
            "%.1f MB".format(b / (1024f * 1024))

        else ->
            "%.2f GB".format(b / (1024f * 1024 * 1024))
    }
}
...
private fun DeviceTab(
    ...
    val ramPct =
        if (state.ramTotal > 0)
            (ramUsed.toFloat() / state.ramTotal * 100).toInt()
        else
            0
            ...
            GlassCard {
                Column(Modifier.padding(16.dp)) {
                    ...
                    Row(
                        ...
                        Text(
                            "Free: ${fmtRam(state.ramAvail)}",
                            color = AzlukSuccess,
                            fontSize = 12.sp
                        )

                        Text(
                            "Total: ${fmtRam(state.ramTotal)}",
                            color = AzlukOnSurface,
                            fontSize = 12.sp
                        )
                    }
...
@Composable
private fun WifiNetCard(
    net: WifiNet
) {
    val signalPct = WifiManager.calculateSignalLevel(
        net.rssi,
        100
    )
    ...
    Column(
        ...
        GlassCard {
            Column(Modifier.padding(16.dp)) {
                ...
                Text(
                    "Current cache: ${fmtRam(state.cacheSize)}",
                    color = AzlukOnSurface,
                    fontSize = 13.sp
                )
                ...
                if (state.cleanedBytes > 0) {
                    Text(
                        "Last cleaned: ${fmtRam(state.cleanedBytes)}",
                        color = AzlukSuccess,
                        fontSize = 13.sp
                    )
                }
