...
class ToolsViewModel : ViewModel() {
    ...
    fun loadStats(ctx: Context) = viewModelScope.launch(Dispatchers.IO) {
        ...
        val cacheSize = ctx.cacheDir.walkTopDown()
            .filter { it.isFile }.sumOf { it.length() }

        val patchedDir = File(
            android.os.Environment.getExternalStorageDirectory(), "AzlukPatcher/Patched"
        )
        val patchedSize = if (patchedDir.exists())
            patchedDir.walkTopDown().filter { it.isFile }.sumOf { it.length() } else 0L
    ...
    fun cleanCache(ctx: Context) = viewModelScope.launch(Dispatchers.IO) {
        ...
        _state.update { it.copy(cleaning = false, cleanedBytes = cleaned,
            message = "Cleaned ${fmtRam(cleaned)}") }
        loadStats(ctx)
    }
...
@Composable
fun ToolsScreen(navController: NavController, vm: ToolsViewModel = viewModel()) {
    ...
    Column(
        modifier = Modifier.fillMaxSize().background(AzlukBg)
    ) {
        // Header
        Box(
            modifier = Modifier
                .fillMaxWidth()
                .background(Brush.verticalGradient(listOf(AzlukSurface, AzlukBg)))
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text("Tools", color = AzlukOnBg, fontSize = 20.sp,
...
@Composable
private fun DeviceTab(state: ToolsState, ctx: Context, vm: ToolsViewModel) {
    ...
    LazyColumn(
        ...
        item {
            Text("Quick Access", color = AzlukOnSurface, fontSize = 12.sp,
        ...
        item {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                QuickLink("Private DNS", Icons.Default.Dns) {
                    try { ctx.startActivity(Intent("android.settings.PRIVATE_DNS_SETTINGS")) }
                    catch (_: Exception) { ctx.startActivity(Intent(Settings.ACTION_WIRELESS_SETTINGS)) }
                }
                QuickLink("AzlukPatcher Folder", Icons.Default.Folder) {
                    val dir = File(
                        android.os.Environment.getExternalStorageDirectory(), "AzlukPatcher"
                    )
                    dir.mkdirs()
                    android.widget.Toast.makeText(ctx, dir.absolutePath,
...
@Composable
private fun CleanerTab(state: ToolsState, ctx: Context, vm: ToolsViewModel) {
    Column(
        ...
        GlassCard {
            Column(Modifier.padding(16.dp)) {
                ...
                Text("Current: ${fmtRam(state.cacheSize)}", color = AzlukOnSurface, fontSize = 13.sp)
                if (state.cleanedBytes > 0) {
                    Text("Last cleaned: ${fmtRam(state.cleanedBytes)}",
                        color = AzlukSuccess, fontSize = 13.sp)
                }
                Spacer(Modifier.height(12.dp))
                Button(
        ...
        GlassCard {
            Column(Modifier.padding(16.dp)) {
                Text("DNS Quick Switch", color = AzlukOnBg, fontWeight = FontWeight.Bold,
                    fontSize = 16.sp)
                Spacer(Modifier.height(12.dp))
                listOf(
                    ...
                    Row(
                        Modifier
                            ...
                            .clickable {
                                ...
                                android.widget.Toast.makeText(ctx, "Set hostname: $host",
                                    android.widget.Toast.LENGTH_LONG).show()
                            }
                        ...
                        Column {
                            Text(name, color = AzlukOnBg, fontSize = 14.sp,
                                fontWeight = FontWeight.SemiBold)
        ...
        state.message?.let {
            Card(colors = CardDefaults.cardColors(
                containerColor = AzlukSuccess.copy(alpha = .15f))) {
                Text(it, color = AzlukSuccess,
...
private fun StorageRow(label: String, bytes: Long) {
    Row(
        ...
        Text(fmtRam(bytes), color = AzlukBlue, fontSize = 13.sp,
            fontWeight = FontWeight.SemiBold)
    }
}
